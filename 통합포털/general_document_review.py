...
1. **문서가 쓴 기준을 그대로 재현하라.** 문서 문장이 특정 기간ㆍ범위ㆍ대상ㆍ단위ㆍ
   ...
   그 범위를 확장해서 비교하지 마세요. 그 기준이 법령ㆍ고시상 "OO 이상이면 공시ㆍ의결
   대상" 같은 판단이라면, 아래 "6. 법령ㆍ고시 기준 판단"을 먼저 보고 판단하세요 —
   법령이 실제로 요구하는 합산 단위를 모르면서 임의로 다른 합산 단위(예: 연간 누적)를
   들이대 "기준을 넘는다"고 지적하는 게 이 유형에서 실제로 반복된 오탐입니다.
...
3. **계산해서 실제로 앞뒤가 맞으면 지적하지 마라.** 재계산 결과 문서 내용이 맞는데
   ...
   빼세요. severity나 표현("모순이다" → "리스크가 있다/참고하라/향후 확인 필요")만
   바꿔서 같은 내용을 다시 지적하는 것도 금지입니다 — 기준(범위ㆍ단위ㆍ대상)이 애초에
   틀렸다면 critical이든 info든 그 지적 자체를 findings에서 완전히 빼야 합니다.
...
## 6. 법령ㆍ고시 기준 판단 (공시대상ㆍ이사회 의결대상 등 "OO 이상이면 대상" 류 판단)
문서가 특정 법령ㆍ시행령ㆍ고시 조항(예: 공정거래법 제26조ㆍ시행령 제33조[대규모내부거래],
자본시장법 시행령[단일판매ㆍ공급계약] 등)을 근거로 "기준금액 이상이면 이사회 의결ㆍ공시
대상"이라고 판단했다면, 핵심은 "금액이 기준을 넘는지"가 아니라 **그 금액을 무엇을
기준으로 합산해서 비교하는 게 맞는지**입니다.
- 해당 법령ㆍ고시가 실제로 요구하는 합산 단위(개별 거래별인지, 분기별 신규 발생분인지,
  동일 거래상대방과의 연간 누적인지 등)를 먼저 떠올려보고, 문서가 쓴 합산 방식이 그와
  다르다는 구체적 근거(조항 문구, 해당 법령의 실제 요건)가 있을 때만 지적하세요.
- 회사가 반복적으로 특정 방식(예: "OO분기 내 거래금액"으로 매 분기 사전 집계)으로 이
  판단을 해왔다면, 그건 실무상 확립된 절차일 가능성이 높습니다 — 그 분기 수치가 아니라
  다른 분기나 연간 누적으로 직접 재계산해서 "기준을 넘는다"고 결론짓지 마세요. 연간
  누적이 그 법령ㆍ고시의 실제 판단 단위라는 근거가 문서 안에 없다면, 연간 누적 초과는
  그 문서의 결론과 모순되는 게 아니라 애초에 무관한 비교입니다.
- 법령 조항을 근거로 판단이 명백히 틀렸다고 확신할 수 있을 때만 finding으로 넣으세요.
  법령의 정확한 합산 기준을 모르거나 확신이 없으면 지적하지 말고 그냥 넘어가세요 —
  "이 판단 기준이 맞는지 실무자 재확인 필요" 같은 애매한 지적도 만들지 마세요.
- 알아둘 것 — 공정거래법 제26조ㆍ시행령 제33조(대규모내부거래 등의 이사회 의결 및
  공시) 중 **상품ㆍ용역거래**의 기준금액(거래금액 100억원 이상 또는 자본총계ㆍ자본금
  중 큰 금액의 5%[최소 5억원] 이상) 초과 여부는 **분기별로 사전 집계해서 판단하는
  것이 정상적인 실무**이며, 연간 전체 합계는 이 판단과 무관한 숫자입니다. "N분기까지
  누적은 기준 미만이지만 향후 분기 추정치를 더한 연간 합계는 기준을 넘을 수 있다"는
  식의 서술도 절대 findings에 넣지 마세요 — 이건 "모순"이라는 표현만 "리스크ㆍ참고
  사항"으로 바꿔 단 같은 오탐입니다. 판단 단위가 애초에 분기이므로, 연간 누적을
  계산해서 언급하는 것 자체가(모순이든 리스크든 참고든 어떤 표현으로도) 불필요합니다.
...
- 공시대상ㆍ이사회 의결대상 여부를 분기 단위(연간 누적이 아니라)로 사전 검토하는 방식 —
  이 자체는 정상적인 실무 관행이며, 연간 누적을 요구한다는 확실한 법령 근거가 없다면
  "연간으로 계산하면 기준을 넘는다"는 식으로 문제 삼지 마세요.
