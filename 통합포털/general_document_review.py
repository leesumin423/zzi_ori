...
1. **문서가 쓴 기준을 그대로 재현하라.** 문서 문장이 특정 기간ㆍ범위ㆍ대상ㆍ단위ㆍ
   ...
   대상" 같은 판단이라면, 아래 "6. 법령ㆍ고시 인용 검증"의 절차대로 그 조항을 실제로
   찾아 대조한 뒤에만 판단하세요 — 인용된 조항이 무엇을 요구하는지 확인하지 않은 채
   임의의 합산 기준을 들이대 "기준을 넘는다"고 지적하는 게 이 유형에서 실제로 반복된
   오탐입니다.
...
## 6. 법령ㆍ고시 인용 검증 (공시대상ㆍ이사회 의결대상 등 "법령상 OO 기준" 판단)
문서가 특정 법령ㆍ시행령ㆍ고시 조항을 근거로 판단(예: "OO법 제OO조에 따라 기준금액
이상이면 이사회 의결ㆍ공시 대상")했다면, 아래 절차로 **그 조항 자체를 근거로 매번
직접 검증**하세요 — 특정 사례의 결론을 외워서 재사용하지 말고, 어떤 법령ㆍ조항이
나오든 이 절차를 그대로 적용하세요.

1. 문서가 인용(또는 근거로 삼고 있음이 분명한)한 법령ㆍ조항이 정확히 무엇인지 먼저
   특정하세요(법령명, 조항 번호, 관련 시행령ㆍ고시가 있다면 그것까지).
2. 그 조항이 실제로 규정하는 내용을 아는 지식을 근거로 판단하세요 — 적용 대상, 기준
   금액, **금액을 산정하는 기준(개별 거래별인지, 특정 기간 누적인지, 그 기간이 분기인지
   연간인지, 동일 거래상대방과의 합산 여부 등)**, 예외 사유.
3. 문서가 그 조항을 실제로 어떻게 적용했는지(어떤 금액을 어떤 기간ㆍ범위로 합산해서
   기준과 비교했는지) 확인하고, 2에서 파악한 조항의 실제 요건과 대조하세요.
4. **대조 결과 문서의 적용 방식이 조항의 실제 요건과 다를 때만** 지적하세요 — 이때는
   반드시 어느 법령ㆍ조항의 어떤 요건과 어떻게 다른지 quote/근거에 구체적으로 적으세요
   (예: "OO법 시행령 제OO조는 OO 기준인데 문서는 OO로 계산함"). 조항의 정확한 내용을
   확신하지 못하면, 문서에 없는 다른 산정 기준(예: 문서가 분기 기준으로 판단했는데
   임의로 연간 누적을 만들어 대는 것)을 들이대 비교하지 말고, 그 부분은 아예 지적하지
   마세요 — 법령을 정확히 모르는 채로 추측성 지적을 만드는 것보다 지적하지 않는 편이
   낫습니다.
5. 대조 결과 문서의 적용 방식이 조항의 실제 요건과 같다면(기간ㆍ범위ㆍ표현이 다르게
   보일 뿐 조항이 요구하는 산정 방식대로 정확히 적용한 것이라면) findings에 절대
   넣지 마세요. severity를 낮추거나("모순" 대신 "리스크ㆍ참고사항") 표현만 바꿔서
   같은 내용을 다시 지적하는 것도 금지입니다 — 조항 요건과 일치한다고 판단했다면
   그 판단 자체를 findings에 남기지 말고 그냥 넘어가세요.
