  ...
  return `
    ...
    <div class="rule-flow">
      <div class="rule-flow-box">
        ...
        <div class="rule-flow-value num-hl">1/1 또는 7/1</div>
      </div>
      <div class="rule-flow-arrow">~</div>
      <div class="rule-flow-box">
    ...
    <h4>감자(액면병합) 등으로 반기 중 변경상장한 경우</h4>
    <p>동양우ㆍ동양2우B는 2026.7.20 감자(액면병합, 구주 2주→신주 1주)로 <b>변경상장</b>했습니다
    (신규상장이 아닙니다) — 반기(7/1~12/31)는 그대로 유지되지만, 병합 전ㆍ후는 1주가
    표상하는 가치ㆍ발행주식총수 자체가 달라져서(병합 전 발행주식총수 525,231주 →
    병합 후 262,615주, 정확히 절반) 거래량(주식수)도 그대로 합산할 수 없습니다. 그래서
    병합 전(7/1~7/2) 거래량에 병합비율(0.5)을 곱해 <b>신주 환산 거래량</b>으로 바꾼 뒤
    반기 시작일(7/1)부터 그대로 합산합니다 — 액면분할ㆍ병합 시 가격ㆍ거래량 연속성을
    맞추는 증권시장의 표준적인 소급환산 방식과 같은 원리입니다.</p>
    <h4>"실거래 페이스 기준 반기 전체 예상"(참고용)</h4>
    <p>위 잠정치는 경과월수(달력상 개월 수, 일부 지난 달도 1개월로 계산)로 나누기 때문에
    변경상장 직후에는 실제 거래 강도보다 낮게 보일 수 있습니다. 그래서 변경상장 종목에
    한해, 변경상장 이후(신주 기준) <b>일평균 거래량</b>이 반기 잔여 매매거래일에도 그대로
    이어진다고 가정하고 반기 전체(6개월)로 환산한 예상치도 작은 글씨로 함께 보여줍니다 —
    어디까지나 지금 페이스가 유지된다는 가정 하의 참고용 추정이며, 실제 반기 종료 시점의
    확정치와 다를 수 있습니다.</p>
    <p class="rule-cite">기준: 유가증권시장 상장규정 제64조제1항제4호(반기 월평균거래량 <b class="num-hl">1만주</b> 기준).
    매일 자동 재계산되며, 반기가 바뀌면 분자ㆍ분모가 자동으로 새 반기 기준으로 초기화됩니다.
    (감자ㆍ병합 시 거래량 환산방법을 명시한 정확한 시행세칙 조문은 KRX 법규서비스 원문으로
    직접 확인하지 못했습니다 — 병합비율은 실제 시가총액ㆍ종가로 역산 검증했으나, 공식
    제출용으로 쓰실 경우 거래소에 조문을 한 번 더 확인하시길 권합니다.)</p>`;
}
