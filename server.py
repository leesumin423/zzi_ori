...
def _theme_market_closed_now() -> bool:
    ...
    is_krx_open()\uc758 \ubc18\ub300(not)\ub97c \uadf8\ub300\ub85c \uc37c\ub358 \uc608\uc804 \ubc84\uc804\uc740 \uc7a5 \uc2dc\uc791 \uc804(\uc790\uc815~09:00)\ub3c4
    "\ub9c8\uac10"\uc73c\ub85c \ucde8\uae09\ud558\ub294 \ubc84\uadf8\uac00 \uc788\uc5c8\ub2e4 \u2014 \uc608\ub97c \ub4e4\uc5b4 \uc624\uc804 8\uc2dc\ub300\uc5d0 \uccab \uc694\uccad\uc774 \ub4e4\uc5b4\uc624\uba74
    fetch_theme_rate()\uac00 \uc544\uc9c1 \uac31\uc2e0 \uc804\uc778 0.00% \ud50c\ub808\uc774\uc2a4\ud640\ub354\ub97c \ub3cc\ub824\uc8fc\ub294\ub370, \uc774\uac78
    "\uc624\ub298 \ub0a0\uc9dc"\ub85c \uce90\uc2dc\uc5d0 \uc800\uc7a5\ud574\ubc84\ub824\uc11c \uc2e4\uc81c\ub85c \uc804\ub0a0 \uc800\uc7a5\ub3fc \uc788\ub358 \uc815\uc0c1\uc801\uc778 \uc804\uc77c \ud655\uc815\uce58\ub97c
    \ub36e\uc5b4\uc37c\ub2e4. \uadf8\ub7ec\uba74 \uc815\uc791 \uc7a5\uc911(09:00~15:30)\uc5d0 "\uc7a5\ub9c8\uac10\uae30\uc900"\uc744 \uc870\ud68c\ud588\uc744 \ub54c, \uce90\uc2dc \ub0a0\uc9dc\uac00
    "\uc624\ub298"\ub85c \ucc0d\ud600\uc788\uc5b4 fetch_theme_change()\uac00 \ubbf8\ud655\uc815 \uac12\uc73c\ub85c \uc624\uc778\ud574 None(N/A) \ucc98\ub9ac\ud558\ub294
    \ubb38\uc81c\ub85c \uc774\uc5b4\uc84c\ub2e4. \uadf8\ub798\uc11c \uc5ec\uae30\uc11c\ub294 "\uc7a5\uc774 \uc2e4\uc81c\ub85c \ub05d\ub09c \ub4a4(15:30 \uc774\ud6c4)\u318d\uc8fc\ub9d0"\ub9cc \ub9c8\uac10\uc73c\ub85c
    \ubcf8\ub2e4 \u2014 \uc7a5 \uc2dc\uc791 \uc804\uc740 \uc808\ub300 \ud3ec\ud568\ud558\uc9c0 \uc54a\ub294\ub2e4."""
    now = datetime.now()
    if now.weekday() >= 5:  # \ud1a0(5)/\uc77c(6)
        return True
...
# 동양우ㆍ동양2우B 감자(액면병합, 구주 2주→신주 1주)로 인한 변경상장일 —
# 2026.7.3~7.16 매매정지, 7.20 변경상장(신규상장 아님). 병합 전(7.1~7.2)과 병합
# 후(7.20~)는 1주가 표상하는 가치ㆍ발행주식총수 자체가 달라져서(병합비율 2:1)
# 거래량(주식수) 단위가 서로 다르다 — 동양우는 병합 전 발행주식총수 525,231주
# (mktcap÷close로 역산, 2026.6.30 기준)가 병합 후 262,615주로 정확히 절반이 됨
# (동양2우B도 동일 비율로 확인). 반기는 그대로 7.1부터지만, 병합 전 거래량에
# ratio(0.5)를 곱해 "신주 환산 거래량"으로 바꿔서 합산해야 한다 — 그대로 더하면
# 서로 다른 단위를 합산하는 셈이라 잠정치가 왜곡된다.
MGMT_SHARE_REORG_DATES = {
    "001525": {"reorg_date": date(2026, 7, 20), "ratio": 0.5},  # 동양우
    "001527": {"reorg_date": date(2026, 7, 20), "ratio": 0.5},  # 동양2우B
}
...
def _streak_status_text(streak_dates: list, latest_dt: date):
    ...
    별표7상 관리종목지정 시기는 "해당 사유 발생일의 다음 날"이다 — 즉 30번째 미달
    매매거래일 그 자체가 지정일이 아니라, 그 다음(달력) 날에 지정된다. 아직 30일이
    안 찼으면 남은 매매거래일 수만큼 평일 기준으로 30번째 날을 추정하고, 거기서 다시
    하루를 더한 게 예상 지정일이다. (공휴일ㆍ추가 매매정지는 미리 알 수 없어 반영하지
    못하는 근사치.)

    반환: (상태문구, 지정일/예상 지정일 문자열 또는 None)
    """
    ...
    day30_dt = _add_business_days(latest_dt, MGMT_STREAK_DAYS - streak)
    designation_dt = day30_dt + timedelta(days=1)
...
def compute_price_status(code: str, history: dict, live_price: int = None) -> dict:
    ...
    관리종목 지정 대상("동전주" 이슈). 같은 항 9호(시가총액 500억원 미달)는 동양
    시총이 훨씬 커서 해당 없어 이 모니터링에서는 계산하지 않는다.
    ...
    eligible_dates = [d for d in trading_dates if d >= MGMT_PRICE_RULE_EFFECTIVE_DATE]
    price_streak_dates = []
    for d_str in reversed(eligible_dates):
        if day_map[d_str]["close"] < MGMT_PRICE_THRESHOLD:
            price_streak_dates.append(d_str)
        else:
            break
