...
3. 메일함 + 전자결재(부서함 완료함/참조·회람함, 개인함 완료함/참조·회람함) 목록을 열어
   각 항목의 제목/본문에서 "까지", "마감", "회신 바랍니다" 같은 표현 근처의 날짜를 찾아냄
4. 오늘 날짜 기준으로 지남/오늘/이번주/차주로 분류
5. 메모지 스타일 팝업으로 표시. 체크해서 "완료 표시 저장"하면 다음부터 안 보임
...
1. `scripts/run_checklist.bat` 을 열어 `PROJECT_DIR` 경로를 실제 설치 경로로 수정
2. Windows 작업 스케줄러 → 작업 만들기
   - 트리거: 매일, 원하는 시간 (예: 오전 8시 30분)
   - 동작: 프로그램 시작 → `run_checklist.bat` 의 전체 경로 지정
3. 참고: 그룹웨어 세션이 짧게 만료되는 경우, 자동 실행 시 로그인이 안 될 수 있습니다.
