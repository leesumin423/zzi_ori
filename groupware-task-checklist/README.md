...
1. **업무 체크리스트** (`main.py`) — 메일함 + 전자결재를 뒤져서 본문에 마감일이 언급된 항목들을 찾아
   **오늘(일간)/주간(이번주~차주)/월간** 으로 분류해 메모지 스타일 팝업으로 보여줍니다.
   이미 기한이 지난 항목은 더 이상 실행할 수 없으므로 팝업에는 표시하지 않습니다.
...
3. 메일함 + 전자결재(부서함 완료함/참조·회람함, 개인함 완료함/참조·회람함) 목록을 열어
   ...
   - 메일함은 목록 화면에 이미 정확한 제목/보낸사람/받은일시가 속성으로 박혀있는 경우
     그 값을 최우선으로 쓰고(본문을 정규식으로 추측하는 것보다 정확함), 없으면 본문에서 추측함
   - 인용된 이전 메일(전달/회신 히스토리)은 잘라내고 현재 메일 작성자가 실제로 쓴 부분만 봄 -
     그래야 인용문 안의 "내일" 같은 표현이나 예전 발신자가 잘못 잡히지 않음
   - `.env`의 `MY_NAME`을 채워두면, "받는사람"이 아니라 "참조"로만 걸린 메일(나에게
     직접 요청된 게 아닌 메일)은 자동으로 제외함
4. 오늘 날짜 기준으로 **오늘(일간) → 주간(이번주~차주 합쳐서) → 월간(그 이후 전부)** 3단으로 간략하게 분류
   (이미 지난 기한은 더 실행할 수 없으므로 팝업에서 제외)
5. 메모지 스타일 팝업으로 표시. 각 항목이 "보낸사람 : 업무내용 : 마감일" 형태로 표시되고,
   그 아래 "원문 보기"를 클릭하면 마감일이 언급된 부분 앞뒤로 본문을 더 넓게 볼 수 있는 창이 떠서
   진짜 마감 요청인지 직접 판단할 수 있음. 체크해서 "완료 표시 저장"하면 다음부터 안 보임.
   나와 상관없는 발신자는 `.env`의 `SENDER_EXCLUDE`에 이름/이메일을 넣으면 아예 제외됨
...
아주 많다면(예: 조회기간 14일 안에 250건 넘게 온다면) 이 값을 늘려주세요.
반대로 실행이 너무 오래 걸린다면 `MAIL_LOOKBACK_DAYS`/`APPROVAL_LOOKBACK_DAYS`를
줄여서(예: 7일) 조회 범위 자체를 좁히는 게 가장 효과적입니다.
